...
full_data <- read.csv("./data/test_film_data_sample.csv")
#possible_data <- read.csv("./data/possible_films.csv")

## adjusting some data types
full_data$Adj_Budget <- as.numeric(as.character(full_data$Adj_Budget))
full_data$Adj_Revenue <- as.numeric(as.character(full_data$Adj_Rev))
full_data$Adj_Weekend_Total <- as.numeric(as.character(full_data$Adj_Revenue))
