...
Using Python, we performed sentiment analysis on movie reviews. We first assessed the model against training and test data in naive_bayes_sentiment_analaysis_test.py. After we were comfortable with our test results, we ran the model on our production data set gathered from the IMDb archive using naive_bayes_sentiment_analaysis.py. We were able to process a total of 21,707 movie reviews. Most of them resulted in positive sentiment. reviews_html_to_csv.py was used to pull film and review data from html files stored locally and place the data we needed into csv's.
